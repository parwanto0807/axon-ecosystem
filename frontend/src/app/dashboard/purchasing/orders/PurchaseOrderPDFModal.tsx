...
interface PurchaseOrderItem { id: string; no: number; description: string; qty: number; unit: string; unitPrice: number; discount: number; amount: number }
interface Vendor { id: string; name: string; code: string; address: string | null; taxId: string | null; phone: string | null; email: string | null }
interface PurchaseOrder {
    id: string; number: string; date: string; status: string
    vendor: Vendor; notes: string | null
    terms: string | null; subtotal: number; taxAmt: number; grandTotal: number
...
const STATUS_CFG: Record<string, { label: string; color: string }> = {
    ...
    PARTIALLY_RECEIVED: { label: 'Partially Received', color: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
...
export default function PurchaseOrderPDFModal({ order, company, onClose }:
    { order: PurchaseOrder; company: Record<string, string>; onClose: () => void }) {
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            // Order meta
            doc.setFont('helvetica', 'normal').setFontSize(8).setTextColor(...gray)
            const hdr = [['PO No.', order.number], ['Tanggal', fd(order.date)], ['Status', sc.label]]
            let hy = y + 4
            hdr.forEach(([k, val]) => {
                doc.setFont('helvetica', 'bold'); doc.text(`${k} :`, W - M - 32, hy, { align: 'right' })
                doc.setFont('helvetica', 'normal').setTextColor(...dark); doc.text(val, W - M, hy, { align: 'right' })
            ...
            doc.text('DIPESAN KEPADA (VENDOR)', M, y); doc.text('INFORMASI TAMBAHAN', col2, y)
            y += 4
            doc.setFont('helvetica', 'bold').setFontSize(9).setTextColor(...dark)
            doc.text(v?.name || '', M, y)
            doc.setFont('helvetica', 'normal').setFontSize(8).setTextColor(...gray)
            let ry = y + 4
            if (v?.address) { const ls = doc.splitTextToSize(v.address, 80); doc.text(ls, M, ry); ry += ls.length * 4 }
            if (v?.phone) { doc.text(`Tel: ${v.phone}`, M, ry); ry += 4 }
            if (v?.email) { doc.text(`Email: ${v.email}`, M, ry) }

            let dy = y
                ;[['Syarat & Ketentuan', order.terms || '']].filter(([, val]) => val).forEach(([k, val]) => {
                    doc.setFont('helvetica', 'bold').setTextColor(...gray); doc.text(`${k}`, col2, dy)
                    doc.setFont('helvetica', 'normal').setTextColor(...dark)
                    const ls = doc.splitTextToSize(`: ${val}`, 70); doc.text(ls, col2 + 22, dy)
            ...
            autoTable(doc, {
                ...
                body: order.items?.map((it, idx) => [idx + 1, it.description, it.qty, it.unit, fr(it.unitPrice), it.discount > 0 ? `${it.discount}%` : '-', fr(it.amount)]) || [],
            ...
            tots.push([`PPN`, `+ ${fr(order.taxAmt || 0)}`])
            doc.setFontSize(8)
            tots.forEach(([k, val]) => {
                doc.setFont('helvetica', 'normal').setTextColor(...gray).text(k, bx, y)
                doc.setFont('helvetica', 'bold').setTextColor(...dark).text(val, W - M, y, { align: 'right' })
                y += 5
            })
            ...
            if (order.notes) {
                ...
                doc.setFont('helvetica', 'bold').setFontSize(7).setTextColor(...gray).text('CATATAN / INSTRUKSI PEMBELIAN', M + 4, y + 5)
            ...
            const sw = (W - M * 2 - 10) / 2
            doc.setFont('helvetica', 'normal').setFontSize(8).setTextColor(...gray)
            doc.text('Pemesanan (Purchasing),', M, y)
            doc.text('Vendor / Supplier,', M + sw + 10, y)
            ...
            doc.text(company.name || '', M, y); doc.text(v?.name || '', M + sw + 10, y);

            // Footer
            const pc = doc.getNumberOfPages()
            for (let i = 1; i <= pc; i++) {
                doc.setPage(i).setFont('helvetica', 'normal').setFontSize(7).setTextColor(...gray)
                doc.text(`Purchase Order diterbitkan oleh ${company.name || 'PT. Axon Ecosystem'}`, M, 290)
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
                ...
                <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50 rounded-t-3xl">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-500 text-sm font-medium">{v?.name}</span>
                    </div>
                    ...
                    <div style={{ background: '#fff', maxWidth: 794, margin: '0 auto', padding: '40px 48px', fontFamily: 'Arial,sans-serif', fontSize: 11, color: '#0f172a', borderRadius: 8, boxShadow: '0 4px 24px rgba(0,0,0,.1)' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #4f46e5', paddingBottom: 18, marginBottom: 0 }}>
                            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                                {company.logo && <img src={`http://localhost:5000${company.logo}`} alt="logo" style={{ height: 60, width: 'auto', objectFit: 'contain' }} />}
                                <div>
                                    ...
                                    <tbody>
                                        {[
                                            ...
                                            <tr key={k}>
                                                ...
                                                <td style={{ textAlign: 'right', color: '#0f172a', paddingBottom: 3, fontWeight: 600 }}>{val}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div style={{ textAlign: 'center', padding: '10px 0 18px', borderBottom: '1px solid #e2e8f0', marginBottom: 20 }}>
                            <span style={{ fontWeight: 900, fontSize: 14, color: '#4f46e5', letterSpacing: 3, textTransform: 'uppercase' }}>Purchase Order</span>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, marginBottom: 24 }}>
                            <div>
                                <div style={{ fontSize: 8, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 2, color: '#94a3b8', marginBottom: 6 }}>Dipesan Kepada (Vendor)</div>
                                <div style={{ fontWeight: 800, fontSize: 13, color: '#0f172a' }}>{v?.name}</div>
                                {v?.address && <div style={{ fontSize: 9, color: '#64748b', marginTop: 4, lineHeight: 1.6 }}>{v.address}</div>}
                                {v?.phone && <div style={{ fontSize: 9, color: '#64748b', marginTop: 3 }}>Tel: {v.phone}</div>}
                            </div>
                            <div>
                                <div style={{ fontSize: 8, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 2, color: '#94a3b8', marginBottom: 6 }}>Informasi Tambahan</div>
                                <table style={{ fontSize: 10, width: '100%', borderCollapse: 'collapse' }}>
                                    <tbody>
                                        {[
                                            ['Syarat & Ketentuan', order.terms]
                                        ].filter(([, val]) => val).map(([k, val]) => (
                                            <tr key={k}>
                                                <td style={{ fontWeight: 700, color: '#475569', paddingRight: 8, paddingBottom: 4, verticalAlign: 'top' }}>{k}</td>
                                                <td style={{ color: '#0f172a', paddingBottom: 4 }}>: {val}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <tbody>
                                {order.items?.map((it, idx) => (
                                    <tr key={idx} style={{ background: idx % 2 === 0 ? '#fff' : '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                                        <td style={{ padding: '7px 10px', textAlign: 'center', color: '#94a3b8', fontWeight: 700 }}>{idx + 1}</td>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
                            <div style={{ width: 300, fontSize: 10 }}>
                                {[
                                    ...
                                    [`PPN`, `+ Rp ${(order.taxAmt || 0)?.toLocaleString('id-ID')}`],
                                ].map(([k, val]) => (
                                    <div key={k} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #f1f5f9', paddingBottom: 5, marginBottom: 5 }}>
                                        <span style={{ color: '#64748b' }}>{k}</span><span style={{ fontWeight: 600, color: '#1e293b' }}>{val}</span>
                                    </div>
                                ))}
                        ...
                        {order.notes && (
                            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6, padding: '10px 14px', marginBottom: 24 }}>
                                <div style={{ fontSize: 8, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 2, color: '#94a3b8', marginBottom: 5 }}>Catatan / Instruksi Pembelian</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, marginTop: 16 }}>
                            {[
                                { label: 'Pemesanan (Purchasing),', name: company.name || 'PT. Axon Ecosystem' },
                                { label: 'Vendor / Supplier,', name: v?.name || '' },
                            ].map((s, i) => (
